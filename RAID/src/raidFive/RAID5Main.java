 ...
 * @author zhz
 */
public class RAID5Main {

	public static void start(Scanner scanner) throws IOException {
		String diskParentPath = "disk/raid5/";// raid5磁盘父路径
		...
		breakPoint: while (true) {
			System.out.println("----模拟磁盘阵列raid5----");
			...
			case "2":
				...
				dataDiskRead.add(parityCkechDisk.get(0));
				RAID5Read raid5Read = new RAID5Read(dataDiskRead, desFilePath);
				for (int i = 0; i < dataDiskRead.size(); i++) {// 有多少个磁盘则启动多少个线程
					new Thread(raid5Read).start();
				}
				break;
			case "0":
				break breakPoint;
